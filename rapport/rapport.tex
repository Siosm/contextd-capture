...
\begin{figure}[hb]
	\centering
	\includesvg{attachements/fonctionnement_avant}{1}
...
\begin{figure}[h]
	\centering
	\includedot{attachements/class}{1}
	\caption{Schéma des classes}
\end{figure}

\subsubsection{Signaux et autres}

Pour rendre contextd conscient de nos modifications nous avons implémenté plusieurs éléments :

\begin{itemize}
	\item Rechargement des programmes avec SIGUSR2, à la suite des settings de contextd.
	\item Nous avons aussi rajouté un mécanisme permettant de nettoyer régulièrement la liste des clients enregistrés dans contextd, car cet action n'était pas présente. Il s'agit simplement d'un thread qui s'exécute de manière synchrone et qui vérifie que les éléments de la liste, repérés par le pid, sont toujours présents dans le fichier /proc/. Si non, il les supprime de la liste. 
\end{itemize}
...
D'autres tests effectués avec Libreoffice, montre que le résultat fonctionne parfaitement. Cependant, les erreurs dues aux refus ne sont pas explicites (Libreoffice considère parfois que le document est endommagé alors que le noyau lui a tout simplement refusé l'accès).

\begin{figure}[h]
	\centering
	\includedot{attachements/fonctionnement_apres}{1}
	\caption{Schéma des classes}
\end{figure}
