...
\subsubsection{L'interface présentée à l'administrateur}
	...
	Pour faciliter l'administration et pour pouvoir intéragir plus aisément avec le noyau, nous avons mis en place deux fichiers dans l'arborescence /proc :
	...
	\begin{itemize}
		...
		\item /proc/contextd/status : Ce fichier est en revanche accessible en lecture/écriture. En lecture, il renvoie le PID de contextd (plus précisément le tgid (threads)). En ecriture, s'il reçoit un 0, il désactive la surveillance des programmes enregistrés et vide la liste. Il est donc impératif que ce dossier ne soit pas accessible par défaut à un utilisateur non privilégié.
	\end{itemize}
...
Ainsi seule "auditsec\_struct" variera suivant les "hooks" qui la rempliront. Nous avons donc créer différentes autres structures pour chaque type de "hook".
...
Voici le code source de cet appel système :
...
Une dernière étapes est cependant nécessaire pour que ces appels systèmes soient pleinement opérationnels. Il faut les déclarer. Pour cela, nous avons ajouter les lignes suivantes dans le fichier \textit{{kernel\_src}/security/auditsec/syscall.c} :
...
Grâce à tout ce qui précède, nous sommes en mesure de stopper les appels systèmes des applications surveillées, de communiquer avec le noyau et de récupérer les informations essentielles pour pouvoir prendre une décision si le noyau donne l'accès ou non à la ressource demandée.

\subsection{Modification de contextd}

Maintenant, voyons les modifications apportées à contextd. Comme cela a été dit précédemment, la principale modification apportée à contextd concerne le mode de communication entre lui et les applications. Initialement, une seule classe existait pour gérer la communication qui ne se faisait que via dbus. Nous avons donc implémenté une classe "pseudo" abstraite `àbstractcontext''.
