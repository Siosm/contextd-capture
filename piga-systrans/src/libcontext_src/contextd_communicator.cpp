...
{
	lastError=error;
	qWarning("%s\n", qPrintable("libcontext error: " + error));
}
...
context_result Contextd_Communicator::contextChanged(QString str)
{
	...
	QDBusReply<QString> reply = iface.call("domain_changed", str);
	if (!reply.isValid())
	{
							...
							QString str)
{
	...
	bool ret = iface.callWithCallback("domain_changed", args, callback,
...
bool Contextd_Communicator::requiredDomain(QString str, QString& res)
{
	if(!isConnected())
		return CONTEXT_ERROR;

	QDBusReply<QString> reply = iface.call("required_domain", str);
	if (!reply.isValid())
	{
		QString error=QObject::tr("LibContext : Call failed: %1\n").arg(reply.error().message());
		setError(error);
...
void Contextd_Communicator::sigHandler(int num)
{
	...
	{
		char buf[151];
		if(context_current_domain(buf, sizeof(buf)-1)==CONTEXT_TRUE)
		{
			QString newContext=buf;
			if(newContext!=currentContext)
			{
				qDebug("SigHandler: current_domain='%s' !\n", buf);
...
QString Contextd_Communicator::currentDomain()
{
	if(!isConnected())
		return "<error>";

	QDBusReply<QString> reply = iface.call("current_domain");
	if (!reply.isValid())
	{
		QString error=QObject::tr("LibContext : Call failed: %1\n").arg(reply.error().message());
		setError(error);
