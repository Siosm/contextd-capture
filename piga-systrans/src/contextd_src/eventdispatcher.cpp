...
#include "plugins/notify/contextdpluginnotify.h"
...
void EventDispatcher::initPlugins()
{
	...
	//Plugins
	if(!addPlugin(&ContextHolder::instance()))
	...
	#ifndef PLUGIN_DISABLE_NOTIFY
		static ContextdPluginNotify cnotify;

		if(!addPlugin(&cnotify))
			sendError("EventDispatcher::EventDispatcher() : Failed to add the plugin Notify !");
		else
			sendNotification("EventDispatcher::EventDispatcher() : Plugin Notify added !");
	#endif
