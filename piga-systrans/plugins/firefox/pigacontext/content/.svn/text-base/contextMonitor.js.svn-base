...
function onDomainChanged()
{
	...
	var cdomain=classPIGA.currentDomain();
	document.getElementById("panel_pigacontext").label = "Current Domain = "+cdomain+"\n";
...
function checkTabsPermissions(new_domain)
{
	...
	for(i=0;i<tab_count;i++)
	{
		try
		{
			...
			dump("requiredDomain: ");
			var required_domain=classPIGA.requiredDomain(tab_uri);
			dump("OK\n");

			if(required_domain!=new_domain)
			{
				dump("	Tab"+i+": URL="+tab_uri+"; "+required_domain+"!="+new_domain+", so, we will redirect it !\n");
...
function onTimer()
{
	var new_domain=classPIGA.currentDomain();
	if(new_domain!=timer_currentContext)
	{
		dump("Context changed from "+timer_currentContext+" to "+new_domain+" !\n");
		checkTabsPermissions(new_domain);
		timer_currentContext=new_domain;
	}
}
...
function registerMyListener()
{
	...
	//Listen to domains changing
	var observeService = getObserverService();
	observeService.addObserver(myListener, "domainChanged", false);
...
var myListener =
{
	...
	*/
	...
	{
		var new_domain=someData;
		dump("Context changed: "+new_domain+"\n");
